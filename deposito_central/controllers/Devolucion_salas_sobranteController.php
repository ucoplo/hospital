...
class Devolucion_salas_sobranteController extends Controller
{
      ...
      try {
       ...
       foreach ($model->renglones as $key => $obj) {
          ...
          if (count($vencimiento)==1){
              ...
              $vencimiento->DT_CODART = $renglon->PR_CODART;
              $vencimiento->DT_FECVEN = $renglon->PR_FECVTO;
              $vencimiento->DT_SALDO = $renglon->PR_CANTID;
              $vencimiento->DT_DEPOSITO = $renglon->PR_DEPOSITO;
          }
    ...
    private function generarPdf($id){
      ...
      $nombre = Yii::$app->params['local_path']['path_deposito_central']."/devoluciones_sobrante/".$nombre;
