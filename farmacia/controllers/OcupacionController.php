...
use farmacia\models\OcupacionBuscar;
...
class OcupacionController extends Controller
{
    ...
    {
        $searchModel = new OcupacionBuscar();
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->save()) {
            return $this->redirect(['view', 'id' => $model->OC_COD]);
        } else {
    ...
    */
    ...
    public function actionQuery($q = null) {
        ...
        $query->select(['OC_COD','OC_DESCRI'])
            ->from(Ocupacion::tableName())
            ->where('OC_DESCRI LIKE "%' . $q .'%" OR OC_COD LIKE "%' . $q .'%"')
            ->orderBy('OC_DESCRI');
        $command = $query->createCommand();
        $data = $command->queryAll();
        foreach ($data as $d) {
            $out[] = ['value' => '[' . $d['OC_COD'] . '] ' . $d['OC_DESCRI'], 'cod' => $d['OC_COD']];
        }
